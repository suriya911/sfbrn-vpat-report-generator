"""End-to-end regression suite — the behavior safety net.

Guards the invariants the whole scoring pipeline depends on:
    ...
    - barrier selection excludes Supports and Not Applicable
...
Runs under pytest with the rest of the suite, and also standalone without
pytest for a quick check on a machine that only has the app installed:

    python -m pytest tests/test_regression.py -q
    python tests/test_regression.py      (no pytest required)
"""
